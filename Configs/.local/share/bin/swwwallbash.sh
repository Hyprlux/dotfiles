...
set -a
source "${wallbashOut}"
if [ -f "${hyprluxThemeDir}/theme.dcol" ] && [ "${enableWallDcol}" -eq 0 ]  ; then
    source "${hyprluxThemeDir}/theme.dcol"
    echo "[theme] Overriding dominant colors from \"${hyprluxTheme}\""
    echo "[note] Remove \"${hyprluxThemeDir}/theme.dcol\" to use wallpaper dominant colors"
fi
...
fn_wallbash () {
    local tplt="${1}"
    [ -f "${hyprluxConfDir}/hyprlux.conf" ] && source "${hyprluxConfDir}/hyprlux.conf"
    # Skips the the template declared in ./hyprlux.conf
...
if [ "${enableWallDcol}" -eq 0 ] && [[ "${reload_flag}" -eq 1 ]] ; then
    ...
    echo ":: deploying ${hyprluxTheme} colors :: ${dcol_mode} wallpaper detected"
    mapfile -d '' -t deployList < <(find "${hyprluxThemeDir}" -type f -name "*.theme" -print0)

    while read -r pKey ; do
        fKey="$(find "${hyprluxThemeDir}" -type f -name "$(basename "${pKey%.dcol}.theme")")"
...
if [ "${enableWallDcol}" -eq 0 ]; then
    colorScheme="$({ grep -q "^[[:space:]]*\$COLOR-SCHEME\s*=" "${hyprluxThemeDir}/hypr.theme" && grep "^[[:space:]]*\$COLOR-SCHEME\s*=" "${hyprluxThemeDir}/hypr.theme" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} || 
                    grep 'gsettings set org.gnome.desktop.interface color-scheme' "${hyprluxThemeDir}/hypr.theme" | awk -F "'" '{print $((NF - 1))}')"
