...
[ -z "${hyprluxTheme}" ] && echo "ERROR: unable to detect theme" && exit 1
get_themes
...
Theme_Change()
{
    local x_switch=$1
    for i in ${!thmList[@]} ; do
        if [ "${thmList[i]}" == "${hyprluxTheme}" ] ; then
            if [ "${x_switch}" == 'n' ] ; then
...
if ! $(echo "${thmList[@]}" | grep -wq "${themeSet}") ; then
    themeSet="${hyprluxTheme}"
fi

set_conf "hyprluxTheme" "${themeSet}"
...
sed '1d' "${hyprluxThemeDir}/hypr.theme" > "${confDir}/hypr/themes/theme.conf" # Useless and already handled by swwwallbash.sh but kept for robustness
gtkTheme="$(
{ grep -q "^[[:space:]]*\$GTK[-_]THEME\s*=" "${hyprluxThemeDir}/hypr.theme" && grep "^[[:space:]]*\$GTK[-_]THEME\s*=" "${hyprluxThemeDir}/hypr.theme" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} || 
grep 'gsettings set org.gnome.desktop.interface gtk-theme' "${hyprluxThemeDir}/hypr.theme" | awk -F "'" '{print $((NF - 1))}'
)"

gtkIcon="$(
{ grep -q "^[[:space:]]*\$ICON[-_]THEME\s*=" "${hyprluxThemeDir}/hypr.theme" && grep "^[[:space:]]*\$ICON[-_]THEME\s*=" "${hyprluxThemeDir}/hypr.theme" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} ||  
grep 'gsettings set org.gnome.desktop.interface icon-theme' "${hyprluxThemeDir}/hypr.theme" | awk -F "'" '{print $(NF - 1)}'
)"
...
"${scrDir}/swwwallpaper.sh" -s "$(readlink "${hyprluxThemeDir}/wall.set")"
