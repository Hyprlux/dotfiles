...
export hyprluxConfDir="${confDir}/hyprlux"
export cacheDir="$HOME/.cache/hyprlux"
...
get_themes()
{
    ...
    done < <(find "${hyprluxConfDir}/themes" -mindepth 1 -maxdepth 1 -type d)
...
if [ -z "${hyprluxTheme}" ] || [ ! -d "${hyprluxConfDir}/themes/${hyprluxTheme}" ] ; then
    get_themes
    hyprluxTheme="${thmList[0]}"
fi

export hyprluxTheme
export hyprluxThemeDir="${hyprluxConfDir}/themes/${hyprluxTheme}"
export wallbashDir="${hyprluxConfDir}/wallbash"
...
set_conf()
{
    local varName="${1}"
    local varData="${2}"
    touch "${hyprluxConfDir}/hyprlux.conf"

    if [ $(grep -c "^${varName}=" "${hyprluxConfDir}/hyprlux.conf") -eq 1 ] ; then
        sed -i "/^${varName}=/c${varName}=\"${varData}\"" "${hyprluxConfDir}/hyprlux.conf"
    else
        echo "${varName}=\"${varData}\"" >> "${hyprluxConfDir}/hyprlux.conf"
    fi
}
