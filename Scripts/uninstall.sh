#!/usr/bin/env bash
#|---/ /+----------------------------------+---/ /|#
#|--/ /-| Script to remove Hyprlux configs |--/ /-|#
#|-/ /--| Prasanth Rangan                  |-/ /--|#
#|/ /---+---------------------------------+/ /---|#
...
.: WARNING :: This will remove all config files related to Hyprlux :.

please type "DONT REMOVE" to continue...
EOF

read promptIn
[ "${promptIn}" == "DONT REMOVE" ] || exit 0
...
[ -d "$HOME/.config/hyprlux" ] && rm -rf "$HOME/.config/hyprlux"
[ -d "$HOME/.cache/hyprlux" ] && rm -rf "$HOME/.cache/hyprlux"
...
Remove Hyprlux related backups/icons/fonts/themes manually from these paths
