...
dcolDir="${confDir}/hyprlux/wallbash/Wall-Dcol"
...
Fav_Theme_Dir="${Theme_Dir}/Configs/.config/hyprlux/themes/${Fav_Theme}"
[ ! -d "${Fav_Theme_Dir}" ] && print_prompt -r "[ERROR] " "'${Fav_Theme_Dir}'" -y " Do not Exist" && exit 1

config=$(find "${dcolDir}" -type f -name "*.dcol" | awk -v favTheme="${Fav_Theme}" -F 'Wall-Dcol/' '{gsub(/\.dcol$/, ".theme"); print ".config/hyprlux/themes/" favTheme "/" $2}')
restore_list=""
...
restore_list+="Y|Y|\${HOME}/.config/hyprlux/themes/${Fav_Theme}|theme.dcol|hyprland\n"
fi
...
# populate wallpaper
Fav_Theme_Walls="${confDir}/hyprlux/themes/${Fav_Theme}/wallpapers"
